...
describe('ofx parsing', () => {
  test('should parse an ofx statement request', () => {
    ...
    const bankStatementRequest = messageSets[1]
      .cast<BankingRequestMessageSet>()
      .getStatementRequest()
      .getMessage()
  ...
  test('should parse an ofx response', () => {
    ...
    expect(message.getAvailableBalance().getAmount()).toBe(5250.0)
    expect(message.getLedgerBalance().getAmount()).toBe(5250.0)

    const transactions = message.getTransactionList().getTransactions()
    expect(transactions).toHaveLength(3)
    expect(transactions[0].getAmount()).toBe(200.0)
    expect(transactions[1].getAmount()).toBe(150.0)
    expect(transactions[2].getAmount()).toBe(-100.0)
  })

  xtest('should download profile info from a bank', async () => {
